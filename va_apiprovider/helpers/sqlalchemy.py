...
RELATION_BLACKLIST = ('query', 'query_class', '_sa_class_manager','_decl_class_registry')
...
def assign_attributes(sanic_restapi_model, **kwargs):
    ...
    """
    cls = type(sanic_restapi_model)
    for field, value in kwargs.items():
        if not hasattr(cls, field):
            msg = '{0} has no field named "{1!r}"'.format(cls.__name__, field)
            raise TypeError(msg)
        setattr(sanic_restapi_model, field, value)
